...
fn main() -> io::Result<()> {
    ...
    let prev_server_address = "127.0.0.1:8082";
    let next_server_address = "127.0.0.1:8083";
    ...
    let mut remainging_to_fail = 200;
    loop {
        ...
        match socket.recv_from(&mut buffer) {
            Ok((n, sender_address)) => {
                ...
                if sender_address.to_string() == next_server_address || sender_address.to_string() == prev_server_address{
                    ...
                    // to be adjusted to check for a PORT number when testing on a network scale
                    println!("Server 1: Received a request from {}: {}", sender_address, message);
                    if counter == turn{
            ...
            Err(e) => {
                eprintln!("Server 1: Error receiving data: {}", e);
            }
        }
    }
}
